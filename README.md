...
The iOS app proactively keeps the camera's latest GPS cache updated; the camera does not request GPS immediately before each shutter release. Background Link, Low Power Mode, CoreBluetooth restoration/pending reconnect, and best-effort Background App Refresh are available for physical-device testing.
